describe("Panel Report Tests", () => {
  describe("Report Tests", () => {
    ...
    it("Report: BALANCE / 2023", () => {
      ...
      cy.intercept("GET", "/api/panel/report?&type=BALANCE&year=2023", { fixture: "panel/report.json" });
      ...
      cy.get(`[data-row-key="1"] > :nth-child(10) a`).
        should('have.css', 'color', "rgb(245, 34, 45)");

      cy.get(`[data-row-key="2"] > :nth-child(9) > a`).
        should('have.css', 'color', "rgb(22, 119, 255)");

      cy.get(`[data-node-key="2"]`).click();

      cy.get("#apexchartsline").
        should('have.css', 'height', '350px');
      cy.get("#apexchartsbar").
        should('have.css', 'height', '350px');
    });
  });
});
